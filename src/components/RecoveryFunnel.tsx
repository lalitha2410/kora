 ...
 * funnels, an ops team doesn't. Four ordered stages, ONE flat accent color
 ...
 * from. Bar label text is dark (colors.ink), not white — colors.accent is a
 * light, high-chroma green, and white-on-bright-green is the pairing that
 * reads poorly; dark text on a bright fill is the legible direction here.
 *
 * Card surface matches HeroStats' `inkRaised` deliberately — this and the
 * two hero cards are the same "raised card on the dark shell" surface, not
 * three different one-off treatments.
 */
...
export function RecoveryFunnel({ stages, colors }: { stages: FunnelStage[]; colors: BrandColors }) {
  ...
  return (
    <div className="rounded-lg px-4 py-3" style={{ backgroundColor: colors.inkRaised }}>
      <div className="mb-2.5 flex items-baseline justify-between">
        ...
        <p className="text-[10.5px]" style={{ color: `${colors.paper}66` }}>
          carts, this session
        </p>
      </div>
        ...
        {stages.map((stage, i) => {
          ...
          return (
            <div key={stage.label} className="flex items-center gap-2.5">
              <span className="w-16 shrink-0 text-[10.5px] font-medium" style={{ color: `${colors.paper}bb` }}>
                {stage.label}
              </span>
              <div
                className="relative h-5 min-w-0 flex-1 rounded-sm"
                style={{ backgroundColor: `${colors.paper}14` }}
              >
                <div
                  className="flex h-5 items-center justify-end rounded-sm px-1.5 transition-[width] duration-500"
              ...
              <span className="w-9 shrink-0 text-right font-mono text-[10px]" style={{ color: `${colors.paper}66` }}>
